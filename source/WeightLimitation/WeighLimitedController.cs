    ...
    internal static class WeighLimitedController
    {
            ...
            bool current_result, ref string errorMessage, MechLabPanel mechlab)
        {
            ...
            if (component is IWeightLimited)
            {
                ...
                if (mechlab.activeMechDef.Chassis.Tonnage < limit.MinTonnage || mechlab.activeMechDef.Chassis.Tonnage > limit.MaxTonnage)
                {
                    if(limit.MinTonnage == limit.MaxTonnage)
                        errorMessage = string.Format("{0} designed for {1}t mech", component.Description.Name, limit.MinTonnage);
                    else
                        errorMessage = string.Format("{0} designed for {1}t-{2}t mech", component.Description.Name, limit.MinTonnage, limit.MaxTonnage);
                    return false;
                }
            }
            return true;
            ...
            foreach (var component in mechDef.Inventory.Where(i => i.Def != null).Select(i => i.Def).OfType<IWeightLimited>())
            {
                if (component.MinTonnage < mechDef.Chassis.Tonnage || component.MaxTonnage > mechDef.Chassis.Tonnage)
                {
                    ...
                    if (component.MinTonnage == component.MaxTonnage)
                        errors[MechValidationType.InvalidInventorySlots].Add(string.Format("{0} designed for {1}t mech", item.Description.Name, component.MinTonnage));
                    else
                        errors[MechValidationType.InvalidInventorySlots].Add(string.Format("{0} designed for {1}t-{2}t mech",
