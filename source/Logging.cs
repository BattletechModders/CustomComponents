...
internal static class Logging
{
    ...
    private static readonly ILog LOG = Logger.GetLogger(nameof(CustomComponents), LogLevel.Debug);
    ...
    internal sealed class LevelLogger
    {
        ...
        [Conditional("CCDEBUG")]
        public void LogDebug(DType type, string message)
        {
            if (Control.Settings.DebugInfo.HasFlag(type))
            {
                log.LogAtLevel(level, message);
            }
        }
        ...
        [Conditional("CCDEBUG")]
        public void LogDebug(DType type, string message, Exception e)
        {
            if (Control.Settings.DebugInfo.HasFlag(type))
            {
                log.LogAtLevel(level, message, e);
        ...
        [Conditional("CCDEBUG")]
        public void LogDebug(DType type, string message, params object[] objs)
        {
            if (Control.Settings.DebugInfo.HasFlag(type))
            {
                log.LogAtLevel(level, string.Format(message, objs));
            }
        }
    }
}